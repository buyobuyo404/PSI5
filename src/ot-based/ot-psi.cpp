...
uint32_t otpsi(role_type role, uint32_t neles, uint32_t pneles, uint32_t *elebytelens, uint8_t **elements,
               uint8_t ***result, uint32_t **res_bytelen, crypto *crypt_env, CSocket *sock, uint32_t ntasks,
               double epsilon,
    ...
    if (DETAILED_TIMINGS) {
        ...
        cout << "Time for domain hashing:\t" << fixed << std::setprecision(2) << getMillies(t_start, t_end) << " ms"
             << endl;
    }
...
uint32_t otpsi_client(uint8_t *elements, uint32_t neles, uint32_t nbins, uint32_t pneles,
    ...
    cout << "stash size: " << stashsize << endl;
...
void otpsi_server(uint8_t *elements, uint32_t neles, uint32_t nbins, uint32_t pneles, uint32_t elebitlen,
                  uint32_t maskbitlen,
    ...
    oprg_server(hash_table, nbins, neles * NUM_HASH_FUNCTIONS, nelesinbin, outbitlen, maskbitlen, crypt_env, sock,
                ntasks, masks);
    if (DETAILED_TIMINGS) {
        gettimeofday(&t_start, NULL);
    }
...
void *
otpsi_query_hash_table(void *ctx_tmp) {//GHashTable *map, uint8_t* elements, uint8_t** result, uint32_t elebytelen,
...
uint32_t otpsi_find_intersection(uint32_t **result, uint8_t *my_hashes,
                                 uint32_t my_neles, uint8_t *pa_hashes, uint32_t pa_neles, uint32_t hashbytelen,
                                 uint32_t *perm) {
    ...
    for (i = 0, intersect_ctr = 0; i < pa_neles; i++) {
        ...
        if (g_hash_table_lookup_extended(map, (void *) &tmpbuf, NULL, (void **) &tmpval)) {
            if (keys_stored > 1) {
                tmpbuf = 0;
                memcpy((uint8_t *) &tmpbuf, pa_hashes + i * hashbytelen + sizeof(uint64_t),
                       hashbytelen - sizeof(uint64_t));
